...
- [ ] T005 Verify Audio: Speak into Mic, hear delay/echo on Speaker (Manual Test) <!-- id: 14 -->

## Phase 2: ESP-NOW Communication
**Goal:** Establish reliable wireless link between two units.
- [ ] T006 Initialize WiFi in Station Mode & ESP-NOW in `main/wifi_transport.c` <!-- id: 15 -->
- [ ] T007 Define `audio_packet_t` struct (Header + SeqNum + Payload) in `main/app_config.h` <!-- id: 16 -->
- [ ] T008 Implement TX Task: Send dummy packets with counter in `main/wifi_transport.c` <!-- id: 17 -->
- [ ] T009 Implement RX Callback: Receive packets & log Sequence Numbers in `main/wifi_transport.c` <!-- id: 18 -->
...
- [ ] T011 Implement RingBuffer for Audio-WiFi decoupling in `main/ring_buffer_lib.c` <!-- id: 20 -->
- [ ] T012 Connect Audio Task to RingBuffer (Push Mic Data) in `main/main.c` <!-- id: 21 -->
- [ ] T013 Connect WiFi Task to RingBuffer (Pop Data -> Packetize -> Send) in `main/wifi_transport.c` <!-- id: 22 -->
- [ ] T014 Implement RX Assembly (Receive -> Jitter Buffer/Ring Buffer -> I2S Write) in `main/wifi_transport.c` <!-- id: 23 -->
